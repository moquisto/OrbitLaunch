...
def _desired_header() -> list[str]:
    return [
        ...
        "upper_pitch_time_3", "upper_pitch_angle_3",
...
def log_iteration(phase: str, iteration: int, params: OptimizationParams, results: dict):
    ...
    upper_pitch_sorted = sorted(
        [
            ...
            [params.upper_pitch_time_3, params.upper_pitch_angle_3],
        ],
        key=lambda p: float(p[0]),
    )
    ...
    with open(LOG_FILENAME, "a", newline="") as f:
        ...
        writer.writerow([
            ...
            f"{params.upper_pitch_time_3:.1f}", f"{params.upper_pitch_angle_3:.1f}",
            ...
            f"{_float_or_nan(results.get('eval_t_sim_s', float('nan'))):.3f}",
            f"{_float_or_nan(results.get('eval_altitude_m', float('nan'))):.2f}",
            f"{_float_or_nan(results.get('eval_speed_mps', float('nan'))):.3f}",
            f"{_float_or_nan(results.get('eval_vr_mps', float('nan'))):.3f}",
            f"{_float_or_nan(results.get('eval_fpa_deg', float('nan'))):.3f}",
